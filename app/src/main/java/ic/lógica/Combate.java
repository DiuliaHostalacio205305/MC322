...
public class Combate {
    ...
    public void fluxoCombate() throws InterruptedException{
        ...
        /*TURNO DO HEROÍ */
            ...
            if(nRodada != 1){
                while (acao !=0 && heroi.getCafeina() > 0 && inimigo.getVida() > 0){
                    System.err.println(Cores.COLOR_GREEN + "\nÉ sua vez novamente!" + Cores.COLOR_RESET);
                    Thread.sleep(1000);
                    printaStats(heroi, inimigo);
                    textoEscolha(heroi, tabuleiro);
                    acao = scanner.nextInt();
            ...
            System.out.println(Cores.COLOR_GREEN + "\nVida de " + heroi.getName() + " = " + heroi.getVida() + "/" + heroi.getvidaMax() + Cores.COLOR_RESET);
            Thread.sleep(1000);
        ...
        if(heroi.getVida() <= 0){
            System.out.println(Cores.COLOR_RED + "\nOh nãooo... \nVocê reprovou :( \nPelo menos da para tentar de novo semestre que vem!" + Cores.COLOR_RESET);
        }
        if(inimigo.getVida() <= 0){
                System.out.println(Cores.COLOR_GREEN + "\nParabéns!\nVocê passou na disciplina:)\nAproveite as suas férias!" + Cores.COLOR_RESET);
        }
   ...
   /**
    ...
    public static void infosCafeina(Heroi heroi) throws InterruptedException{
        System.out.println(Cores.COLOR_YELLOW + "Cafeína disponível: " + heroi.getCafeina() + Cores.COLOR_RESET);
        Thread.sleep(1000);
        if(heroi.getCafeina() == 0){
            System.out.println(Cores.COLOR_RED + "\nAcabou sua cafeína :(" + Cores.COLOR_RESET);
        }
    }
    ...
    public void escolhaAcoesHeroi(int acao) throws InterruptedException{
        ...
        if (acao == 0){ //escolheu encerrar o turno
            System.out.println(Cores.COLOR_RED + "Certeza?! Tá bom... Encerrando turno" + Cores.COLOR_RESET);
        } 
            ...
            if (heroi.getCafeina() - custo < 0){ //AQUI ao invés de ser -1 precisa ser - custo da carta, n terminei
                System.out.println(Cores.COLOR_RED + "Hm... Você não tem cafeína suficiente pra isso... Escolha uma outra ação (ou tenta pegar um café lá no IC, se a máquina te permitir)" + Cores.COLOR_RESET);
                ...
                System.out.println(Cores.COLOR_RED + "Você não escolheu uma ação válida. Por favor, digite um número entre 0, 1, 2, 3 ou 4 e aperte Enter" + Cores.COLOR_RESET);
            }
        }
    }
    ...
    public static void textoEscolha(Heroi heroi, Tabuleiro tabuleiro) throws InterruptedException{
        Thread.sleep(1000);
        System.out.println("Escolha uma carta (1-4) ou 0 para encerrar:\n(Caso você não tenha cafeína suficiente para nenhuma ação, vai dormir!! (vulgo, encerre seu turno)" + Cores.COLOR_CYAN);
        tabuleiro.exibirMao();
        Thread.sleep(1000);
        System.out.println(Cores.COLOR_YELLOW + "- Cafeína disponível: " + heroi.getCafeina() + Cores.COLOR_RESET+ "\n");
